...
При создании базы данных используется переменная окружения `PATH_TO_DATABASES`, которая представляет собой путь, до
места, где будут храниться базы данных. Принимает как относительный, так и абсолютный путь. При запуске main_server 
нужно обязательно добавить. Пример:
```bash
export PATH_TO_DATABASES=path/to/databases
```

По традиции, Clion может помочь с этим. 
...
Чтобы пробросить переменную окружения, нужно добавить в поле Environment variables пункт `PATH_TO_DATABASES`.
